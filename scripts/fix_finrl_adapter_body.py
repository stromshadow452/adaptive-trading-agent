...
# Match the function and capture ONLY its body (lines indented under def)
pat = re.compile(
    r"(?ms)^def\s+finrl_signal_adapter\s*\(\s*candidate\s*,\s*policy\s*,\s*finrl_cfg\s*\)\s*:\s*\n(?P<body>(?:^[ \t].*\n|^\n)*)"
)

m = pat.search(src)
if not m:
    print("ERROR: finrl_signal_adapter function not found"); sys.exit(1)
...
# Re-indent with 4 spaces and ensure trailing newline
indented = "".join(("    " + line if line else "") + "\n" for line in new_body.splitlines())

new_src = src[:m.start("body")] + indented + src[m.end("body"):]
p.write_text(new_src, encoding="utf-8")
print("fixed: finrl_signal_adapter body (4-space indents)")
